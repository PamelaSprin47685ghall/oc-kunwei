...
describe('getGreperConfig', () => {
  test('returns greper agent with basher tool and read permission', () => {
  ...
  test('prompt warns against using basher for modifications', () => {
    const cfg = getGreperConfig();
    expect(cfg.agents.greper.prompt).toContain('Do NOT use basher');
  });
});
...
describe('createGreperTool', () => {
  ...
  test('sends query to child session', async () => {
    const ctx = mockCtx();
    const greper = createGreperTool(ctx);
    await greper.execute({ query: 'Find the auth middleware' }, {} as any);
  ...
  test('uses greper agent', async () => {
    const ctx = mockCtx();
    const greper = createGreperTool(ctx);
    await greper.execute({ query: 'search test' }, {} as any);
    const promptArg = ctx.client.session.prompt.mock.calls[0][0];
